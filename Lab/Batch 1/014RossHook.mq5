...
void OnTick(){
   ...
   {      
      ...
      if(TimeCurrent()>Start){
         ...
         //Inside Bar
         if(Price[2].high>Price[1].high && Price[2].low<Price[1].low){
            //Down Trend
            if(Price[2].close<Price[3].low){
...
void executeBuyStop(double entry){
   ...
   if(Ask>entry) return;
   positionSize(MathAbs(entry-Price[1].low)/_Point);
   trade.BuyStop(Lot, entry, _Symbol, Price[1].low, NULL, NULL, 0, text);
}

void executeSellStop(double entry){
   double Bid = NormalizeDouble(SymbolInfoDouble(_Symbol, SYMBOL_BID), _Digits);
   if(Bid<entry) return;
   positionSize(MathAbs(entry-Price[1].high)/_Point);
   trade.SellStop(Lot, entry, _Symbol, Price[1].high, NULL, NULL, 0, text);     
}
