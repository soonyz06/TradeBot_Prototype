...
input string Custom =""; //
input int l = 20; //Band Length
input int s = 1000; //SlPoints
input int t = 30; //Time Exit
...
string Trade_Stop = string(Z)+":50:00"; //End Time
...
void OnInit(){
   ...
   BOHandler = iBands(_Symbol, Timeframe, len, 0, 2, PRICE_CLOSE);
   Comment("ID: ", Magic);
}
...
void OnTick(){
   ...
   {      
      ...
      if(TimeCurrent()>Start){
         ...
         CopyBuffer(BOHandler, 0, 0, 3, Middle);
         CopyBuffer(BOHandler, 1, 0, 3, Upper);
         CopyBuffer(BOHandler, 2, 0, 3, Lower);
         checkPositions();
         timeUpdate();
         
         //Entry
         if(Price[2].close>Upper[2] && Price[1].close<Upper[1]){
            if(sellPosition==0 && buyPosition==0)
            executeSell();
            
         }
         if(Price[2].close<Lower[2] && Price[1].close>Lower[1]){
   ...
   if(today2!=yesterday2){
      if(TimeCurrent()>Stop){
         ...
         if(Price[0].close>Upper[0]){
            closePosition(POSITION_TYPE_BUY);
         }
         if(Price[0].close<Lower[0]){
...
void executeBuyStop(double entry){
   ...
   trade.BuyStop(Lot, entry, _Symbol, Ask-SlPoints*_Point, NULL, ORDER_TIME_SPECIFIED, Expiration, text);
}
...
void executeSellStop(double entry){
   ...
   positionSize(SlPoints);
   trade.SellStop(Lot, entry, _Symbol, Bid+SlPoints*_Point, NULL, ORDER_TIME_SPECIFIED, Expiration, text);     
}
