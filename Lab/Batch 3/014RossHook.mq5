...
int SlPoints = 0;
input int band = 3000; //Band-Limiter
...
void executeBuyStop(double entry){
   ...
   if(SlPoints==0){
      ...
      trade.BuyStop(Lot, entry, _Symbol, POI, NULL, NULL, 0, text);
   }
   else{
...
void executeSellStop(double entry){
   ...
   if(SlPoints==0){
      ...
      trade.SellStop(Lot, entry, _Symbol, POI, NULL, NULL, 0, text);
   }
   else{
      positionSize(SlPoints);
